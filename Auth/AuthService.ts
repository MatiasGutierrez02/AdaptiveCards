...
export default class AuthService {
    ...
    private static CACHE_KEY: string;
    ...
    public static async initialize(context: AdaptiveCardExtensionContext): Promise<void> { 
        AuthService.context = context;
        AuthService.loginUser = context.pageContext.user.loginName;
        AuthService.isTeamsDesktop = context.deviceContext !== 'WebView' ? true : false;
        if (AuthService.isTeamsDesktop) {
            await app.initialize();
        }
        await CacheService.initialize(context);
        this.CACHE_KEY = context.pageContext.user.loginName + "-" + CLIENT_ID;
    ...
    private static interactionRequired(): Promise<string> {
      ...
      return this.msalInstance
        .acquireTokenPopup(loginPopupRequest)
          .then((tokenResponse) => {
              this.setLoginStatus("user_is_authenticated", tokenResponse.expiresOn.getTime(), tokenResponse.accessToken);
  ...
  public static async getAccessToken(scopes?: string[]): Promise<string> {
    ...
    public static async msTeamsAuthenticate(): Promise<void> {
        ...
        return authentication.authenticate({
            ...
            successCallback: (result) => {
                ...
                this.setLoginStatus("user_is_authenticated", this.expiresOn, this.accessToken);
                return result;
            },
    ...
    public static async isUserAuthenticated(): Promise<boolean> {        
        const status = await CacheService.getStatus(this.CACHE_KEY);
        // sessionStorage.getItem(this.SESSION_KEY); 
        return status === "user_is_authenticated" ? true : false;            
    }

    private static setLoginStatus(status: string, exp_date: number = 0, token: string = ""): void {
        CacheService.saveData(this.CACHE_KEY, status, exp_date, token);
        //sessionStorage.setItem(this.SESSION_KEY, status);
    }

    public static async getToken(): Promise<string> {        
        if (this.isTeamsDesktop) {
            // Check data in properties
            if (this.accessToken !== "" && this.expiresOn > new Date().getTime()) {
                return this.accessToken;
            }
            // Check data in Cache
            const loginInfo = await CacheService.getData(this.CACHE_KEY);
            if (loginInfo && loginInfo.token && loginInfo.exp_date > new Date().getTime()) {
                this.accessToken = loginInfo.token;
                this.expiresOn = loginInfo.exp_date;
                return this.accessToken;
            }
            // Get new Token
